...
import { editCommentWO, postCommentWO } from "../util/APIUtils";
import { toast } from "react-toastify";
import LikeWorkOut from "./LikeWorkOut";
import Share from "./Share";

const CommentBoxWO = props => {
  ...
  const handlePostComment = async () => {
    ...
    try {
      const response = await postCommentWO({
        workoutId: props.id,
        text: comment,
  ...
  const handleEditComment = async () => {
    try {
      console.log(comment);
      const response = await editCommentWO(props.comment.id, {
        ...props.comment,
        text: comment
      });
      if (response) {
