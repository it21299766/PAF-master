...
import { updateMealplan , updateSharedPost } from "../util/APIUtils";

export default function MealplanEditModal({
                                        open,
                                        setOpen,
                                        mealplan,
                                        refetchMealplans,
  ...
  const [message1, setMessage1] = useState(mealplan.message1 );
  const [description, setDescription] = useState(mealplan.description );
  const [postType, setPostType] = useState(mealplan.postType );
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await updateMealplan(mealplan.id, { ...mealplan, 
        description,
         message1, 
  ...
  const updateShared = async () => {
    try {
      const response = await updateSharedPost(mealplan.id, {
        ...mealplan,
        description,
        message1
      });
