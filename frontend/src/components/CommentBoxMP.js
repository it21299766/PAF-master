...
import { editCommentMP, postCommentMP } from "../util/APIUtils";
import { toast } from "react-toastify";
import LikeMealPlan from "./LikeMealPlan";
import Share from "./Share";

const CommentBoxMP = props => {
  ...
  const handlePostComment = async () => {
    ...
    try {
      const response = await postCommentMP({
        mealplanId: props.id,
        text: comment,
  ...
  const handleEditComment = async () => {
    try {
      console.log("meal EditComment try "+ comment)
      const response = await editCommentMP(props.comment.id, {
        ...props.comment,
        text: comment
      });
      if (response) {
      ...
      console.log("meal EditComment "+error)
