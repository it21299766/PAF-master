...
import { deleteCommentMP } from "../util/APIUtils";
import CommentBoxMP from "./CommentBoxMP";

const CommentMP = ({ comment, ...rest }) => {
  const [commentState, setCommentState] = useState(comment);
  const [editComment, setEditComment] = useState(false);

  const handleDeleteComment = async () => {
    try {
      const response = await deleteCommentMP(comment.id, rest.currentUser.id);
      if (response) {
