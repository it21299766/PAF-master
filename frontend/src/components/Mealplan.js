...
import { deleteMealplan } from "../util/APIUtils";
import MealplanEditModal from "./MealplanEditModal";
import CommentBoxMP from "./CommentBoxMP";
import CommentMP from "./CommentMP";

const Mealplan = props => {
  const [mealplan, setMealplan] = useState(props);
  const [open, setOpen] = useState(false);
  const [detailsExpanded, setDetailsExpanded] = useState(false); // State for tracking whether details are expanded
  const { shared } = props;

  const updateComment = comment => {
    setMealplan({ ...mealplan, comments: [comment, ...mealplan.comments] });
  };

  const updateDeletedComment = id => {
    setMealplan({
      ...mealplan,
      comments: mealplan.comments.filter(comment => comment.id !== id)
    });
  };

  const handleDeleteMealplan = async () => {
    try {
      const response = await deleteMealplan(mealplan.id);
      if (response) {
        toast("Post deleted successfully", { type: "success" });
        props.refetchMealplan();
      }
    } catch (error) {
  ...
  const isOwner = mealplan.userId === props.currentUser.id;
    ...
    <div className="editor mx-auto my-6 w-10/12 flex flex-col border border-black max-w-2xl">
      <article className="overflow-hidden rounded-lg shadow transition hover:shadow-lg">
        ...
        <div className="container mx-auto">
          <div className="-m-1 flex flex-wrap md:-m-2">
            {mealplan.fileUrl_WO?.length > 0 &&
              mealplan.fileUrl_WO.map((fileUrl, i) => {
        ...
        <div className="bg-white p-4 sm:p-6 ">
          <button
           ...
           focus:ring-gray-300 font-medium rounded-t-full text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800
        ...
        <div className="bg-white border-t text-black ">
          <CommentBoxMP {...props} shared={shared} updateComment={updateComment} />
        </div>
        <div className="bg-white border-t text-black ">
          {mealplan.comments.length > 0 &&
            mealplan.comments.map(comment => (
              <CommentMP
                key={comment.id}
                comment={comment}
      ...
      {open && (
        <MealplanEditModal
          ...
          setOpen={setOpen}
          mealplan={mealplan}
          refetchMealplans={props.refetchMealplans}
        />
      )}
    </div>
  );
