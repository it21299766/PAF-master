...
import { createMealplan } from "../util/APIUtils";
// import ImageUploader from './ImageUploader';

const CreateMealplan = ({ currentUser, refetchMealplans }) => {
  const initialState = {
    description: "",
    message1: "",
    postType: "",
  ...
  const [mealplan, setMealplan] = useReducer((prevState, newState) => {
    return { ...prevState, ...newState };
  }, initialState);

  const handleDescription = event => {
    setMealplan({ description: event.target.value });
  };

  const handleMessage1 = event => {
    setMealplan({ message1: event.target.value });
  };
  const handleCategoryChange = event => {
    setMealplan({ postType: event.target.value });
  };
  ...
  const submitPost = async () => {
    ...
    if (!mealplan.postType) {
      toast("Post Type is mandatory", { type: "error" });
      return;
    }
    if (mealplan.fileUrl_WO.length === 0) {
  ...
  const handleImageChangeW = event => {
    ...
    Promise.all(imagePromises)
      .then(base64Images => {
        setMealplan({ fileUrl_WO: base64Images });
      })
      .catch(error => {
  ...
  return (
    <>
      <div
        ...
        />
        <div className="flex">
          {mealplan.fileUrl_WO.map((fileUrl_WO, index) => {
