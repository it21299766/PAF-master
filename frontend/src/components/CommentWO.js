...
import { deleteCommentWO } from "../util/APIUtils";
import CommentBoxWO from "./CommentBoxWO";

const CommentWO = ({ comment, ...rest }) => {
  const [commentState, setCommentState] = useState(comment);
  const [editComment, setEditComment] = useState(false);

  const handleDeleteComment = async () => {
    try {
      const response = await deleteCommentWO(comment.id, rest.currentUser.id);
      if (response) {
  ...
  return editComment ? (
    ...
    <ul className="divide-y divide-black dark:divide-black">
      <li className="p-2 sm:p-2 ">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-left font-medium text-slate-950 truncate dark:text-slate-950">
          ...
          {isOwner && (
            <div className="inline-flex items-right text-base font-semibold text-gray-900">
              ...
              <div
                ...
                className="inline-flex justify-center p-2 text-black rounded-lg cursor-pointer hover:text-white hover:bg-black dark:text-black dark:hover:text-white dark:hover:bg-black"
              >
