...
import { disLikeMealplan, getMealplanById, likeMealplan } from "../util/APIUtils";
import { toast } from "react-toastify";

const LikeMealPlan = props => {
  ...
  const handleDislike = async () => {
    try {
      const mealplan = await getMealplanById(props.id);
      const like = mealplan.likes.filter(
        like => like.userId === props.currentUser.id
      )[0];

      await disLikeMealplan({
        likeId: like.id
      });
      setLiked(false);
      setLikeCount(prevCount => prevCount - 1);
